...
class Main extends Component {
  render() {
    ...
    if (this.context.gameState.isTie) {
      ...
      } else if (this.context.gameType === GAME_TYPES.VERSUS_COMPUTER) {
        if (this.context.gameState.position === "") {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `It's your turn`;
          else textInfo = `It's bot turn`;
        } else {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `Bot win!`;
          else textInfo = `You win!`;
        }
      } else {
        if (this.context.gameState.position === "") {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `It's Bot1 turn`;
          else textInfo = `It's Bot2 turn`;
        } else {
          if (this.context.playerTurn === PLAYER_TURNS.HUMAN) textInfo = `Bot1 win!`;
          else textInfo = `Bot2 win!`;
        }
      }
    }
